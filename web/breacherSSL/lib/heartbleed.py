...
def attack(ip, port, tlsversion, starttls='none', timeout=5):
    ...
    if starttls == 'none':
        ...
        print '[-] Connecting to ' + str(ip) + ':' + str(port) + ' using ' + tlslongver + ' with STARTTLS'
    sys.stdout.flush()
    ...
    try:
        ...
        if starttls == 'smtp':
            ...
            if not 'STARTTLS' in res:
                print >> sys.stderr, '[ERROR] STARTTLS does not appear to be supported.\n'
                sys.stderr.flush()
                return False
            s.send('starttls\n')
            recvall(s, buffer_size)
        ...
        while True:
            ...
            if type is None:
                print >> sys.stderr, '[ERROR] The server closed the connection without sending the ServerHello. This might mean the server does not support ' + tlslongver + ' or it might not support SSL/TLS at all.\n'
                sys.stderr.flush()
                return False
        ...
        while True:
            ...
            if type == 24:
                if len(payload) > 3:
                    if starttls == 'none':
                        print '[Vulnerable] Heartbeat response was ' + str(len(payload)) + ' bytes instead of 3! ' + str(ip) + ':' + str(port) + ' is vulnerable over ' + tlslongver + ''
                    else:
                        print '[Vulnerable] Heartbeat response was ' + str(len(payload)) + ' bytes instead of 3! ' + str(ip) + ':' + str(port) + ' is vulnerable over ' + tlslongver + ' with STARTTLS'
            ...
            if type == 21:
                print '[-] The server received an alert. It is likely not vulnerable.'
                if verbose: print '[-] Alert Level: ' + alert_levels[ord(payload[0])]
                if verbose: print '[-] Alert Description: ' + alert_descriptions[ord(payload[1])] + ' (see RFC 5246 section 7.2)'
                if verbose: print '[-] Closing connection'
                s.close()
                print ''
                sys.stdout.flush()
        ...
        print >> sys.stderr, '[ERROR] Connection error: ' + str(e.strerror) + ' \n'
        sys.stderr.flush()
        return False
...
def main():
    ...
    if args.starttls not in starttls_options:
        print >> sys.stderr, '[ERROR] Invalid STARTTLS value. Valid values: none, smtp, pop3, imap, ftp.\n'
        parser.print_help()
        sys.exit(1)
    ...
    if 'all' not in versions:
        for v in versions:
            if v not in protocol_name_to_hex:
                print >> sys.stderr, '[ERROR] Invalid SSL/TLS version(s). Valid values: SSLv3, TLSv1.0, TLSv1.1, TLSv1.2.\n'
                parser.print_help()
                sys.exit(1)
    ...
    for host in hosts:
        try:
            ...
            print >> sys.stderr, '[ERROR] Could not resolve an IP address for the given host.\n'
            sys.stderr.flush()
            continue
            ...
            if 'all' in versions:
                if (args.all_versions):
                    ...
                    if not ssl30 and not tls10 and not tls11 and not tls12:
                        if ip == host:
                            print '[OK] ' + host + ':' + str(port) + ' does not appear to be vulnerable to Heartbleed!'
                        else:
                            print '[OK] ' + host + ':' + str(port) + ' (' + str(ip) + ':' + str(port) +') does not appear to be vulnerable to Heartbleed!'
                        sys.stdout.flush()
                else:
