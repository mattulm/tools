        ...
        if ord(recv[0])==22:
            ...
            if fVuln:
                try:
                    ...
                    fVuln = False
            if fVuln:
                print "[!] The target may allow early CCS on %s" % (strVer)
                iVulnCount += 1
            else:
                print "[*] The target rejected early CCS on %s" %  (strVer)
    else:
